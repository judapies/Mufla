...
int8 h=0,l=0,Ajuste=0,Ajusten=0,MediaMovil=10,tiemporecta=0,flag_ganancia=0,Lectura2=0,tiempoup=0,tiempodn=0,Segmentos=0,Segmento=0;
...
int8 Opcion=1,tiempo_esterilizacion=0,ProgramaSeleccionado=0;
int16 tiempos=0,tiempos3=0,tiempoctrl=0,tinicio=0,tiempocalentamiento=0,tiemposetpoint=0,Tp=0,Tc=0;
float PendienteMinutos=0,TempeInicial=20,Tiempo=0,TiempoActual=0,PendienteActual=0,HorasActual=0,MinutosActual=0;
...
float V0=0.0,Tciclo=10.0,Tpulso=0.0,error=0.0,Prom=0.0,Temperature=0.0,tmp=0.0,Ganancia=3.0,desvio=3.0,Coeficiente=150.0;
float Aumento=0.0,Temperatura=0.0,Pendiente=0.0,DifPendiente=0.0,Setpoint=0;
...
float Leer_Sensor(int media)
{
   ...
   Temperature=(1.5275*Prom)-300.0; // Recta dada por T0°C-1200°C y ADC
...
//-------------------------------Control de Calentamiento------------------------------------------------------------//         
   ...
   if(Pendiente==0){// Si Esta realizando una Meseta debe realizar el control JuDifuso
      ...
      if(error>2.0 && error<200 && Aumento<1)
      {
         if(Potencia<90.0)
         {
            if(TiempoControl>=30)
      ...
      PotenciaEntregada=((PendienteMinutos+DifPendiente)*100)/10;
...
// Pendiente determinar si el comportamiento de la mufla es lineal 10°C/min o si tiene un comportamiento diferente.
      ...
      if(Temperatura<450 && Temperatura>=200)
         PotenciaEntregada=((PendienteMinutos+DifPendiente)*100)/8;   
         
      if(Temperatura<=1200 && Temperatura>=450)
         PotenciaEntregada=((PendienteMinutos+DifPendiente)*100)/5;      */
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      {  
         ...
         if(Segmentos>0){
            ...
            Ganancia=Ganancia*(Setpoint/100.0);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(UP)//Si oprime hacia arriba
      {
         glcd_fillscreen(OFF);
               ...
               glcd_line(27+0,55,27+10,55-Temperatura1[ProgramaSeleccionado*10]/30,ON);
 ...
 //----------------Muestra Temperatura--------------------------
         ...
         //if(flag_display==1){
            ...
            //displayFloat((Pendiente*60),60,46,1);}
         //else
      ...
      if(LEFT)//Si oprime boton de Toma.
      {
         if(Pausa!=2)
         {
            delay_ms(500);Menu=30;glcd_fillscreen(OFF);Opcion=1;visual=1;Pausa=3;Menu=1;
         }
      }
