...
short Flanco,Flanco1,Flanco2,Flanco3,flagFinalizo,visual,refresh,refresh2;
float valor,pendiente,b,Ganancia,GananciaIn,b1,b2;
...
uint8 Dato2,Dato3,Dato_Exitoso,Dato_Exitoso2,Menu,MenuAntt,MenuAnt,Menu2,MenuAntt2,z,TipoClave,Segmentos,CicloLibre,Opcion,h,l,ProgramaSeleccionado,conteoTecla;
short Inicio,Inicio2,EstadoPaso,entro,GuardaEEPROM,rampa,Minutos,Horas,flagTemperatura,Test,x1,x10,x100,flag_ajuste,flag_ajuste2;
uint8 Password[4];
uint8 tiempo_esterilizacion,y,hEst,mEst,hEstp,mEstp,Segmento,conteo,unidad,Pausa,controlT;
...
short mensaje,Lectura2,flag_setpoint,flag_tiempocalentamiento,Lectura,flagup,flagdn,flagRampa,flag_calcula,finalizoCiclo,sobrepaso;
short Finalizo,conteoPregunta,grafica,imprime,flag_display,Dibuja,Desconectado,flag_clear3,flag_ganancia,flag_clear2,flagGuardaEstado;
float X[2]={0.0,0.0};
...
float error,Potencia,Tpulsomin,TpulsoMax;
uint16 TiempoActual,MinutosActual,HorasActual,Tiempo,TiempoRampa,tPregunta,b16,tup,tdown,tright,tleft,tSobrepaso;
uint8 dataReady = 0u,Alarma,gananciaIn,segundosCampana,calib;
uint16 result;
float Prom,V0,Temperature,m,m1,m2;
int16 control1,control2,control3,patron1,patron2,patron3;
...
{ 
    ...
    m1=((float)control2-(float)control1)/((float)patron2-(float)patron1);
    b1=(float)control2-(m1*(float)patron2);
    m2=((float)control3-(float)control2)/((float)patron3-(float)patron2);
    b2=(float)control3-(m2*(float)patron3);
    //m3=(1200.0-(float)control3)/(1200.0-(float)patron3);
    //b3=1200.0-(m3*(float)patron3);
    ...
    if(controlT==0){
        ...
        Menu=20;glcd_fillScreen(OFF);
         for(i=0;i<Pasos;i++){
            if(Temperatura1[i+((ProgramaSeleccionado)*Pasos)]==0){
               Segmentos=i;
               break;
            }
            else{
    ...
    if(UP_Read()==0 && DOWN_Read()==0){
        for(;;){ 
            displayTemperatura((uint16)leeTermocupla(MediaMovil),30,45,0,1);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==1){ //Menu Principal.      
         ...
         Ganancia=GananciaIn;
         if(flagGuardaEstado){
            EEPROM_WriteByte(0,495);CyDelay(20);
            flagGuardaEstado=OFF;
         }
         if(Opcion>5)
         {Opcion=1;}
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      {  
         ...
         if(Segmentos>0){
            ...
            glcd_fillScreen(OFF);
            EEPROM_WriteByte(ProgramaSeleccionado,496);CyDelay(20);
            EEPROM_WriteByte(Segmento,497);CyDelay(20);
            strcpy(Menus.Letra," Segmentos");
            displayMenu(Menus.Letra,0,0,0,2);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      ...
      if(tright>30)//Si oprime RIGHT_Read()==0
      {
         BUZZER_Write(1);CyDelay(10);BUZZER_Write(0);CyDelay(30);unidad++;
      }
      
      if(tleft>30)//Si oprime Izquierda
      {
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==9){ //Menu de Tempo Campana.
      
      strcpy(Menus.Letra,"T. CAMP");
      displayMenu(Menus.Letra,30,0,1,2);
      
      if(unidad==4)
      {displayTiempo(Tempocp[3],20,40,1,2);}
      else
      {displayTiempo(Tempocp[3],20,40,0,2);}
      
      if(unidad==3)
      {displayTiempo(Tempocp[2],40,40,1,2);}
      else
      {displayTiempo(Tempocp[2],40,40,0,2);}
      
      if(unidad==2)
      {displayTiempo(Tempocp[1],70,40,1,2);}
      else
      {displayTiempo(Tempocp[1],70,40,0,2);}
      
      if(unidad==1)
      {displayTiempo(Tempocp[0],90,40,1,2);}
      else
      {displayTiempo(Tempocp[0],90,40,0,2);}
      ...
      {  if(Flanco == 0) 
            {BUZZER_Write(1);Flanco = 1;CyDelay(30);Tempocp[unidad-1]++;BUZZER_Write(0);
            }
      }
         else
            {Flanco = 0;}
      ...
      {  if(Flanco2 == 0) 
            {BUZZER_Write(1);Flanco2 = 1;CyDelay(30);Tempocp[unidad-1]--;BUZZER_Write(0);
            }     
      }
         else
            {Flanco2 = 0;}  
      ...
      {  if(Flanco1 == 0) 
            {BUZZER_Write(1);Flanco1 = 1;CyDelay(30);unidad--;BUZZER_Write(0);
            }
      }
         else
            {Flanco1 = 0;}  
      ...
      {
         BUZZER_Write(1);CyDelay(30);BUZZER_Write(0);
         CyDelay(100);Menu=1;glcd_fillScreen(OFF);Opcion=1;unidad=1;
         EEPROM_WriteByte(Tempocp[0],465);
         CyDelay(20);
         EEPROM_WriteByte(Tempocp[1],466);
         CyDelay(20);
         EEPROM_WriteByte(Tempocp[2],467);
         CyDelay(20);
         EEPROM_WriteByte(Tempocp[3],468);
         CyDelay(20);
         Tempoc[0]=0;Tempoc[1]=0;Tempoc[2]=0;Tempoc[3]=0;
      }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(!flagGuardaEstado){
        EEPROM_WriteByte(1,495);CyDelay(20);
        flagGuardaEstado=ON;
      }
      // Muestra imagen de fondo en Display GLCD      
      glcd_imagen(5);      

      if(tup>200 && tdown>200)//Si oprime hacia abajo
      {
      ...
      if (tcHotJnuVolt < BROKEN_TC_THRESHOLD){
         ...
         if(Pausa==0 && DOOR_Read()==0){
            Control=1;
            if(Temperatura>Setpoint+AlTem && AlTem>0 && Pendiente>=0){
               Alarma=5;
            }else{  
               Alarma=10;
 ...
 //----------------Muestra Temperatura--------------------------
      ...
      if(tleft>1000){//Si oprime boton de Toma.
         if(Pausa<3){
            if(Pausa!=2){
      ...
      if(visual==1){
        ...
        */
        char voltage[9];
         ...
         sprintf(Menus.Letra, "%02u  m1:%2.2f b1:%2.2f",calib,m1,b1);
         displayMenu(Menus.Letra,0,0,1,1);
         sprintf(Menus.Letra, "m2:%2.2f b2:%2.2f",m2,b2);
         displayMenu(Menus.Letra,0,8,1,1);
         ...
         if(calib==1){
            sprintf(voltage, "%04u°C",control1);
            glcd_rect(28,18,70,30,YES,ON);
            glcd_text57(30,20,voltage,1,0);
            
            sprintf(voltage, "%04u°C",patron1);
            glcd_rect(78,18,120,30,YES,OFF);
            glcd_text57(80,20,voltage,1,1);            
            
            sprintf(voltage, "%04u°C",control2);
            glcd_rect(28,33,70,45,YES,OFF);
            glcd_text57(30,35,voltage,1,1);
            
            sprintf(voltage, "%04u°C",patron2);
            glcd_rect(78,33,120,45,YES,OFF);
            glcd_text57(80,35,voltage,1,1);
            
            sprintf(voltage, "%04u°C",control3);
            glcd_rect(28,48,70,60,YES,OFF);
            glcd_text57(30,50,voltage,1,1);
            
            sprintf(voltage, "%04u°C",patron3);
            glcd_rect(78,48,120,60,YES,OFF);
            glcd_text57(80,50,voltage,1,1);
            ...
            sprintf(voltage, "%04u°C",control1);
            glcd_rect(28,18,70,30,YES,OFF);
            glcd_text57(30,20,voltage,1,1);
            
            sprintf(voltage, "%04u°C",patron1);
            glcd_rect(78,18,120,30,YES,ON);
            glcd_text57(80,20,voltage,1,0);            
            ...
            sprintf(voltage, "%04u°C",control2);
            glcd_rect(28,33,70,45,YES,ON);
            glcd_text57(30,35,voltage,1,0);
            ...
            sprintf(voltage, "%04u°C",patron2);
            glcd_rect(78,33,120,45,YES,ON);
            glcd_text57(80,35,voltage,1,0);
            ...
            sprintf(voltage, "%04u°C",control3);
            glcd_rect(28,48,70,60,YES,ON);
            glcd_text57(30,50,voltage,1,0);
            ...
            sprintf(voltage, "%04u°C",patron3);
            glcd_rect(78,48,120,60,YES,ON);
            glcd_text57(80,50,voltage,1,0);
            glcd_update();
         ...
         strcpy(Menus.Letra,"Cal1:");
         glcd_text57(0,20,Menus.Letra,1,1);
         
         strcpy(Menus.Letra,"Cal2:");
         glcd_text57(0,35,Menus.Letra,1,1);
         
         strcpy(Menus.Letra,"Cal3:");
         glcd_text57(0,50,Menus.Letra,1,1);
            
         if(LEFT_Read()==0){
            EEPROM_WriteByte(control1,498);
            CyDelay(10);
            EEPROM_WriteByte(patron1,503);
            CyDelay(10);
            EEPROM_WriteByte(convert8(control2,0),499);
            CyDelay(10);
            EEPROM_WriteByte(convert8(control2,1),500);
            CyDelay(10);
            EEPROM_WriteByte(convert8(control3,0),501);
            CyDelay(10);
            EEPROM_WriteByte(convert8(control3,1),502);
            CyDelay(10);
            EEPROM_WriteByte(convert8(patron2,0),504);
            CyDelay(10);
            EEPROM_WriteByte(convert8(patron2,1),505);
            CyDelay(10);
            EEPROM_WriteByte(convert8(patron3,0),506);
            CyDelay(10);
            EEPROM_WriteByte(convert8(patron3,1),507);
            CyDelay(10);
            BUZZER_Write(1);CyDelay(1000);BUZZER_Write(0);
            Menu=100;glcd_fillScreen(OFF);Opcion=1;visual=1;
         }
